...
impl VisualCortex {
    ...
    /// Extrae un ToGen (átomo de realidad) de la entrada visual actual.
    /// En v2.0, esto reemplaza el procesamiento de frames completos.
    /// El sistema busca la característica más saliente (borde, color, movimiento)
    /// y la empaqueta en un ToGen.
    pub fn sample_atom(&self, input: &[f32]) -> ToGen {
        // 1. Simulación de atención visual (Saliency Map)
        // Buscamos el valor máximo en el input (punto de mayor activación)
        ...
        // 2. Codificación a ToGen (Materia)
        // Usamos el índice (posición espacial) y el valor (intensidad)
        // para construir el átomo.
        
        // Quantized: ID de la característica (posición espacial aproximada)
        let quantized = max_idx as u64;
        
        // Raw Data: Intensidad y detalles finos
        // Mapeamos el float a un entero para raw data
        let intensity = (max_val * 1000.0) as u64;
        let raw_data = intensity;

        ToGen::new_materia(quantized, raw_data)
    }
}

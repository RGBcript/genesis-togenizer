...
impl Brain {
    ...
    pub fn new(input_size: usize, output_size: usize) -> Self {
        ...
        // Inicializar Curo (puede fallar si no hay dispositivo, pero asumimos CPU/CUDA)
        let curo = CuroModel::new(&config).expect("Failed to initialize Curo");
    ...
    pub fn think(&mut self, dt: f32) {
        ...
        // 0. Extracción de Átomos (Visual Cortex -> ToGen)
        // En lugar de procesar todo el frame, extraemos el evento más saliente
        let atom = self.visual.sample_atom(&self.current_input);

        // 1. Integración Sensorial (Curo)
        // El átomo entra directamente al integrador
        let mut surprise_factor = 0.0;
        if let Err(e) = self.curo.integrate_atom(&atom) {
            warn!("Error integrating atom in Curo: {}", e);
        } else {
            // Recuperar la última sorpresa registrada
            if let Some(last_surprise) = self.curo.surprise_history.last() {
                surprise_factor = *last_surprise;
            }
        }

        // 2. Corteza Visual: Procesa la entrada cruda (Legacy/Paralelo)
        let visual_out = self.visual.process(&self.current_input, dt);

        // 3. Amygdala: Procesa la salida visual + Factor de Sorpresa
        // Si la sorpresa es alta, la Amygdala recibe un "boost" de señal
        let mut amygdala_input = visual_out.clone();
        if surprise_factor > 0.5 {
            // Inyectar ansiedad: amplificar la señal visual
            for val in amygdala_input.iter_mut() {
                *val *= 1.0 + surprise_factor; 
            }
            warn!("SORPRESA ALTA ({}). Amygdala hipersensible.", surprise_factor);
        }
        
        let amygdala_out = self.amygdala.process(&amygdala_input, dt);
    ...
    fn update_homeostasis(&mut self) {
        // La homeostasis ahora depende inversamente de la sorpresa acumulada
        // Homeostasis = 1.0 - Sorpresa Promedio (0 = Caos, 1 = Zen)
        
        let recent_surprise: f32 = self.curo.surprise_history.iter().rev().take(10).sum();
        let avg_surprise = if self.curo.surprise_history.len() > 0 {
            recent_surprise / 10.0f32.min(self.curo.surprise_history.len() as f32)
        } else {
            0.0
        };
        
        // Actualizamos el estado interno
        self.homeostasis = (1.0 - avg_surprise).clamp(0.0, 1.0);
        
        // Si la homeostasis es muy baja, aplicamos frenado de emergencia
        if self.homeostasis < 0.2 {
             self.apply_homeostasis(); // Frenado de emergencia original
        }
    }
